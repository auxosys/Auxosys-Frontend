...
export default function CareersPage() {
  ...
  useEffect(() => {
    async function fetchJobs() {
      try {
        const res = await fetch("http://localhost:5002/job");
            ...
            {loading ? (
              ...
              <div className="job-card" key={job.id}>
                  ...
                  <div className="job-left">
                    ...
                    <div>
                      ...
                      <div className="job-meta">
                        <span className="job-meta-item"><span className="ico">🏢</span>{job.department}</span>
                        <span className="job-meta-item"><span className="ico">📍</span>{job.work_mode} {job.city ? `- ${job.city}` : ''}</span>
                        <span className="job-meta-item"><span className="ico">🕐</span>{new Date(job.created_at).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="job-detail-inner">
                    <div>
                      ...
                      <ul className="detail-list">
                        {(job.requirements || "").split("\n").map(r => r.trim() ? <li key={r}>{r.replace(/^- /, '')}</li> : null)}
                      </ul>
                    </div>
                    <div>
                ...
                <div className="job-footer">
                  <span className="job-posted">Posted on {new Date(job.created_at).toLocaleDateString()}</span>
