    ...
    return (
        <Link href={`/news/${item.slug}`}
            ...
            <div style={{ position: "relative", overflow: "hidden", minHeight: 360 }}>
                <Image src={item.featured_image_url || item.featuredImage || item.featuredImageUrl || item.image || "/placeholder.jpg"} alt={item.title}
                    fill
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://auxosys-backend.onrender.com'}/news`);
